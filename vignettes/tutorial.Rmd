...
**MDstatsDIAMS** supports **MSstats** report format.
Once a DIA report has been imported into **MSstats** format by using
**MSstatsConvert**, it can be converted into **MDstatsDIAMS** standard report
...
```{r create_mq_annotation, eval=TRUE}
...
annotation_resu_path <- create_mq_annotation_file(
  mq_evidence = mq_ev,
...
if (annotation_resu_path == "") {
  ...
  mq_an <- read.delim(annotation_resu_path)
}
```

**MSstats** converts a **MaxQuant** report into **MSstats** format.

```{r mq_to_msstats, eval=TRUE}
# Convert to MSstats
...
```{r mq_msstats_to_standard, eval=TRUE}
...
```{r skyline_to_msstats, eval=TRUE}
