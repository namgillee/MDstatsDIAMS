...
* Run statisical methods.

```{r run_methods}
# Append a column 'cov_unequal_replicates'
...
# Run methods
# Resu is a list of method results; each method result is a list of
# comparison(== two conditions) results.
...
```{r compute_lfdr}
lfdr_result <- ttest_result

for (method_name in names(lfdr_result)) {
  for (comparison in names(lfdr_result[[method_name]])) {
    p.value <- lfdr_result[[method_name]][[comparison]]$p.value
    protein_id <- lfdr_result[[method_name]][[comparison]]$protein_id

    lfdr <- local_fdr(p.value)
    is_target <- (protein_id %in% known_target_df$UniprotID)

    lfdr_result[[method_name]][[comparison]]$lfdr <- lfdr
    lfdr_result[[method_name]][[comparison]]$is_target <- is_target
  }
}
```
...
for (i in 1:(num_cond - 1)) {
  ...
  df_two_conds0 <- df_subset %>%
    filter(condition %in% c(subconditions[i], subconditions[i + 1]))

  ttest_result0 <- run_ttests(
    df_two_conds0,
    method_names = NULL,
    boot_denom_eps = 0.3,
    base_condition = subconditions[i]
  )

  ## Compute "signed" p-value in precursor level
  for (method in names(ttest_result0)) {
    signed_pvalue_table <- ttest_result0[[method]][[1]] %>% 
      select(experiment, protein_id, precursor_id, p.value, estimate) %>%
      mutate(comparison_1 = subconditions[i],
             comparison_2 = subconditions[i + 1],
...
print(paste(c("AUCs:", aucs), collapse = " "))
```
